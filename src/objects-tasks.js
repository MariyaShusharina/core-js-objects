...
function compareObjects(obj1, obj2) {
  ...
  for (let i = 0; i < entries1.length; i += 1) {
    if (
      entries1[i][0] !== entries2[i][0] ||
      entries1[i][1] !== entries2[i][1]
    ) {
      areEqual = false;
    }
  }
...
function makeWord(lettersObject) {
  ...
  word = [];
  let entries = [];
  entries = Object.entries(lettersObject);
  for (let i = 0; i < entries.length; i += 1) {
    for (let j = 0; j < entries[i][1].length; j += 1) {
      const letterIndex = entries[i][1][j];
      const letter = entries[i][0];
...
function Rectangle(width, height) {
  ...
  const r = Object.create(rectangle);
  r.width = width;
  r.height = height;
  return r;
}
